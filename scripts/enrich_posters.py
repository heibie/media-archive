...
def main():
    ...
    def pause(): time.sleep(args.sleep)

    def find_by_imdb(imdb_id):
        if not imdb_id: return {}
        if imdb_id in find_cache: return find_cache[imdb_id]
    ...
    for m in movies:
        ...
        if not m.get("title"): m["title"] = j_def.get("title")
